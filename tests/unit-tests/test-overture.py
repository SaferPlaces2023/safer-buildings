...
class Test(unittest.TestCase):
    ...
    def test_vicenza_001(self):
        ...
        args = {
            "water": "s3://saferplaces.co/Safer-Buildings/test/vicenza-wd-200mm-1h.tif",
            "building": None,
            "wd_thresh": None,
            "bbox": None,
        ...
        fileout = module_s3.s3_download(
            uri = args['out'],
            fileout = filesystem.tempfilename(suffix='.geojson', prefix='test-vicenza-001-')
        )

        try:
            self.assertTrue(os.path.exists(fileout))
            print(f"Test 'test_vicenza_001' successfully completed.")
        except:
            print(f"Test 'test_vicenza_001' failed. Output file not found: {fileout}")
